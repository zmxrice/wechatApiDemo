...
if($extend!=""){
    if(!in_array($extend,$uptypes)){
		...
		$sql="insert into lostseek(openid,status,title,contact,image,filename, info,addtime, isfinished) values('$openid','$switch','$title','$contact','$image','$temp_name','$info','$mytime',0)";
    	$mysql->runSql( $sql );
		$mysql->closeDb();
    ...
    $sql="insert into lostseek(openid,status,title,contact,info,addtime, isfinished) values('$openid','$switch','$title','$contact','$info','$mytime',0)";
    $mysql->runSql( $sql );
