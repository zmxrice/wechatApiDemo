...
if($extend!=""){
    if(!in_array($extend,$uptypes)){
        ...
        $sql="select filename from lostseek where openid='$openid' and id='$id'";
        $data = $mysql->getData( $sql );
        $s->delete("upload",$data[0][filename]);
        
        $sql="update lostseek set status='$switch', title='$title', contact='$contact', image='$image', filename='$temp_name', info='$info' where openid='$openid' and id='$id'";
    ...
    $sql="update lostseek set status='$switch', title='$title', contact='$contact', image=NULL, filename=NULL, info='$info' where openid='$openid' and id='$id'";
    $mysql->runSql( $sql );
